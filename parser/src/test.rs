...
fn parses_arithmetic_with_precedence() {
    ...
    let ok = matches!(&nodes[0],
        Node::Expr(Expr::Operator { op: Operator::Plus, .. })
    );
...
fn parses_comparisons_as_identifier_operators() {
    ...
    let ok = matches!(&nodes[0],
        Node::Expr(Expr::Operator { op: Operator::Ge, .. })
    );
...
fn parses_and_or_chains_left_associative() {
    ...
    let ok = matches!(&nodes[0],
        Node::Expr(Expr::Operator { op: Operator::Or, left, .. })
            if matches!(left.as_ref(),
                Node::Expr(Expr::Operator { op: Operator::Or, .. })
            )
    );
...
fn parses_methods_vs_field_access() {
    ...
    assert!(matches!(&nodes[0], Node::Expr(Expr::Method(..))));
...
fn parses_when_with_else_if_chain() {
    ...
    .unwrap();
    let ok = matches!(&nodes[0],
        Node::Expr(Expr::If(arms)) if arms.len() == 3
    );
...
fn parses_for_loops_with_ranges() {
    ...
    let ok = matches!(&nodes[0],
        Node::Expr(Expr::For { inclusive: true, .. })
    );
...
fn parses_block_with_statements_and_tail_value() {
    ...
    let ok = matches!(&nodes[0],
        Node::Expr(Expr::Block(items, tail))
...
fn parses_dict_vs_block_disambiguation() {
    ...
    assert!(matches!(&nodes[0], Node::Expr(Expr::Dict(_))));
...
fn parses_late_method_definition() {
    ...
    let ok = matches!(&nodes[0],
        Node::Statement(Statement::Assign { value, .. })
            if matches!(value.as_ref(), Node::Expr(Expr::Fn(..)))
    );
...
fn parses_anonymous_fn_with_expression_body() {
    ...
    let ok = matches!(&nodes[0], Node::Expr(Expr::Fn(..)));
...
fn skips_comments_between_tokens() {
    ...
    assert!(matches!(&nodes[0], Node::Expr(Expr::Operator { op: Operator::Plus, .. })));
}